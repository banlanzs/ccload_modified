...
// normalizeMetadataUserID 将 metadata.user_id 中的 JSON 格式转换为扁平字符串
// Claude API 要求 user_id 为简单字符串（不超过256字符），但某些客户端会发送
// JSON 格式如 {"device_id":"67bad5...","account_uuid":"","session_id":"7581b58b-..."}
// 转换为: "user_67bad5..._account__session_7581b58b-..."
func normalizeMetadataUserID(body []byte) []byte {
	// 快速路径：不含 metadata 则直接返回
	...
	metadataRaw, ok := reqData["metadata"]
	if !ok {
		return body
	}

	var metadata map[string]json.RawMessage
	if err := sonic.Unmarshal(metadataRaw, &metadata); err != nil {
		return body
	}

	userIDRaw, ok := metadata["user_id"]
	if !ok {
		return body
	}

	// 解析 user_id 的值（一个 JSON 字符串）
	var userIDStr string
	if err := sonic.Unmarshal(userIDRaw, &userIDStr); err != nil {
		return body
	}
	...
	if !strings.HasPrefix(trimmed, "{") {
		return body // 已经是普通字符串，无需转换
	}

	// 解析嵌套 JSON：{"device_id":"...","account_uuid":"...","session_id":"..."}
	var userInfo struct {
		DeviceID    string `json:"device_id"`
		AccountUUID string `json:"account_uuid"`
		SessionID   string `json:"session_id"`
	}
	if err := sonic.UnmarshalString(trimmed, &userInfo); err != nil {
		return body // 无法解析，保持原样
	}

	// 构造扁平格式：user_{device_id}_account_{account_uuid}_session_{session_id}
	flatUserID := fmt.Sprintf("user_%s_account_%s_session_%s",
		userInfo.DeviceID, userInfo.AccountUUID, userInfo.SessionID)

	// 写回
	flatRaw, err := sonic.Marshal(flatUserID)
	if err != nil {
		return body
	}
	metadata["user_id"] = flatRaw

	newMetadataRaw, err := sonic.Marshal(metadata)
	if err != nil {
		return body
	}
	reqData["metadata"] = newMetadataRaw
