...
func TestNormalizeMetadataUserID(t *testing.T) {
	...
	tests := []struct {
		name       string
		input      string
		wantUserID string // 期望的 user_id 值；空表示不检查（无 metadata 场景）
		wantNoOp   bool   // 是否期望不修改
	}{
		{
			name:     "无metadata字段_不修改",
			input:    `{"model":"claude-3","messages":[]}`,
			wantNoOp: true,
		},
		{
			name:       "user_id已是扁平字符串_不修改",
			input:      `{"model":"claude-3","metadata":{"user_id":"user_abc123_account_def456_session_ghi789"},"messages":[]}`,
			wantUserID: "user_abc123_account_def456_session_ghi789",
			wantNoOp:   true,
		},
		{
			name:       "user_id为JSON格式_转换为扁平格式",
			input:      `{"model":"claude-3","metadata":{"user_id":"{\"device_id\":\"67bad5\",\"account_uuid\":\"\",\"session_id\":\"7581b58b-1234\"}"},"messages":[]}`,
			wantUserID: "user_67bad5_account__session_7581b58b-1234",
		},
		{
			name:       "user_id为JSON格式_含account_uuid",
			input:      `{"model":"claude-3","metadata":{"user_id":"{\"device_id\":\"aaa111\",\"account_uuid\":\"bbb222\",\"session_id\":\"ccc333\"}"},"messages":[]}`,
			wantUserID: "user_aaa111_account_bbb222_session_ccc333",
		},
		{
			name:     "metadata无user_id_不修改",
			input:    `{"model":"claude-3","metadata":{"custom_field":"value"},"messages":[]}`,
			wantNoOp: true,
		},
		{
			name:     "metadata不是对象_不修改",
			input:    `{"model":"claude-3","metadata":"not_an_object","messages":[]}`,
			wantNoOp: true,
		},
		{
			name:       "真实Claude_Code_v2.1.78数据",
			input:      `{"model":"claude-sonnet-4-20250514","metadata":{"user_id":"{\"device_id\":\"76efe6262d82afb157901eb4176d04f3e66648d3933d9fe096fd280b8d2441c2\",\"account_uuid\":\"\",\"session_id\":\"ce6c5d34-d5f2-45ed-b26f-4098af144ba1\"}"},"messages":[]}`,
			wantUserID: "user_76efe6262d82afb157901eb4176d04f3e66648d3933d9fe096fd280b8d2441c2_account__session_ce6c5d34-d5f2-45ed-b26f-4098af144ba1",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantNoOp {
				// 不修改场景：结果应与输入等价
				var gotMap, inputMap map[string]any
			...
			if tt.wantUserID != "" {
				...
				if !ok {
					t.Fatalf("expected metadata to be object, got %T", got["metadata"])
				}
				userID, ok := metadata["user_id"].(string)
				if !ok {
					t.Fatalf("expected user_id to be string, got %T", metadata["user_id"])
				}
				if userID != tt.wantUserID {
					t.Errorf("user_id = %q, want %q", userID, tt.wantUserID)
				}
			}
		})
	}
